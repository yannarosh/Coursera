run_analysis <- function() {
        ...
        # just for me, remember to comment this out
        setwd("C:/Users/perre/Desktop/Coursera/Data Science/3. Getting and Cleaning Data/Assignment")
        ...
        y_test <- fread(paste0(getwd(), "./assignment_data/UCI HAR Dataset/test/y_test.txt"), header = FALSE, col.names = c("activity_id"))
        y_train <- fread(paste0(getwd(), "./assignment_data/UCI HAR Dataset/train/y_train.txt"), header = FALSE, col.names = c("activity_id"))
        subject_test <- fread(paste0(getwd(), "./assignment_data/UCI HAR Dataset/test/subject_test.txt"), header = FALSE, col.names = c("subject_id"))
        subject_train <- fread(paste0(getwd(), "./assignment_data/UCI HAR Dataset/train/subject_train.txt"), header = FALSE, col.names = c("subject_id"))
        features <- fread(paste0(getwd(),"./assignment_data/UCI HAR Dataset/features.txt"), header = FALSE, col.names = c("feature_id", "feature"))
        X_test <- fread(paste0(getwd(), "./assignment_data/UCI HAR Dataset/test/X_test.txt"), header = FALSE, col.names = features$feature)
        X_train <- fread(paste0(getwd(), "./assignment_data/UCI HAR Dataset/train/X_train.txt"), header = FALSE, col.names = features$feature)
        act_lbl <- fread(paste0(getwd(), "./assignment_data/UCI HAR Dataset/activity_labels.txt"), col.names = c("activity_id", "activity"))
        ...
        # select only rows with mean or std.
